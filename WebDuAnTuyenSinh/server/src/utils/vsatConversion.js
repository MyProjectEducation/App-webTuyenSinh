...
// 2. Ma trận bách phân vị (Lấy từ bảng của Bộ GD&ĐT, đồng bộ 100% với React Client)
const VSAT_MATRIX = {
    'Toán': [
    ...
    'Tiếng Anh': [
        { rank: '3%', a: 131.0, b: 150.0, c: 7.75, d: 9.75 },
...
/**
 * Chuyển đổi điểm V-SAT sang thang THPT 10 điểm (Đồng bộ chuẩn xác)
...
exports.convertVsatToThpt = (monHocAlias, diemVsat) => {
    ...
    let score = Number(diemVsat);
    if (isNaN(score) || score <= 0) return 0;
    if (score > 150) score = 150;

    // 4. Thuật toán quét tìm phân vị (a < x <= b)
    const range = matrix.find(r => score > r.a && score <= r.b);

    if (!range) {
        // Fallback: nếu điểm rơi xuống dưới mốc thấp nhất của ma trận
        const lowestRange = matrix[matrix.length - 1];
        if (score <= lowestRange.a) return lowestRange.c;
        throw new Error(`[VSAT_ERROR] Điểm ${score} của môn ${standardName} không nằm trong bất kỳ khoảng quy đổi nào.`);
    }

    const { a, b, c, d } = range;
    
    // Đề phòng lỗi chia cho 0
    if (a === b) return c;
    if (score === a) return c;
